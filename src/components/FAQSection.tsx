    ...
    <div className="space-y-3 sm:space-y-4">
      {FAQS.map((f) => (
        <details key={f.q} className="group rounded-2xl p-[1px] bg-gradient-to-br from-sky-400/40 to-blue-400/40 shadow-sm open:shadow-md transition">
          <summary className="list-none select-none cursor-pointer rounded-2xl bg-white/90 px-4 py-3 sm:px-5 sm:py-4 backdrop-blur ring-1 ring-black/5 text-sm font-semibold text-gray-900 flex items-center justify-between">
            <span>{f.q}</span>
          ...
          <div className="rounded-b-2xl bg-white/90 px-4 pb-4 sm:px-5 sm:pb-5 text-sm text-gray-700 ring-1 ring-black/5 -mt-1">
            {f.a}
          </div>
        </details>
      ))}
